...
static uint8_t ReceivedDataBuffer[128];
...
static void RadioMoveToRxStateWithFifosClear(void);
static void RadioReceivedData(void);
/*======================================================================================*/
...
//	uint8_t bufeklen;
	...
	for(uint8_t i=0;i<receivedDataLen+2;i++)ReceivedDataBuffer[i]=0;

	RD_RadioReceiveData(ReceivedDataBuffer, receivedDataLen);

	recMsgLen=(ReceivedDataBuffer[receivedDataLen-2]<<8) + ReceivedDataBuffer[receivedDataLen-1];

	if(recMsgLen == receivedDataLen-2){
		Kom_Send(ReceivedDataBuffer,receivedDataLen-2);
	}
	else{
		Kom_Send("RCV ERROR\r\n",11);
	}

//	bufeklen=sprintf(bufek,"RSSI: %d\r\n",RssiDuringReceiving);
//	Kom_Send(bufek,bufeklen);
...
void RD_RadioConfig(void)
{
	...
	RD_RadioSendWriteTransaction(TEMP_VALUE_OFFSET_13_REG, 0x00);
	  ...
	  /* Crystal load capacitance setting */
	  RD_RadioSendWriteTransaction(CRISTAL_OSCILATOR_LOAD_CAP_09_REG, 0x78); // 12 pF
	  ...
	  RD_RadioSendWriteTransaction(CLK_RECOVERY_TIMING_LOOP_GAIN0_25_REG, 0x06);
	  RD_RadioSendWriteTransaction(RESERVED_2A_REG, 0x45);
	  RD_RadioSendWriteTransaction(AGC_OVERRIDE_69_REG, 0x60);
	  ...
	  RD_RadioSendWriteTransaction(FREQUENCY_DEVIATION_72_REG, 0x70);

	  /* Carrier frequency setting */
	  RD_RadioSendWriteTransaction(FREQ_BAND_SEL_75_REG, 0x73);
	  RD_RadioSendWriteTransaction(FREQ_NOMINAL_CARRIER_FREQ1_76_REG, 0x77);
	  RD_RadioSendWriteTransaction(FREQ_NOMINAL_CARRIER_FREQ2_77_REG, 0x10);

	  /* Hopping setting */
	  RD_RadioSendWriteTransaction(FREQ_HOPPING_CH_SEL_79_REG, 0x00); // 0x00 no frequency hopping
	  RD_RadioSendWriteTransaction(FREQ_HOPPING_STEP_SIZE_7A_REG, 0x00); // 0x00 no frequency hopping
...
* 	SPI:
*
* 	SDN			- PA9 -> LOW = ON, HIGH = Shutdown
* 	NIRQ		- PA8 -> LOW = Interrupt
* 	SPI2_CLK	- PB13
* 	SPI2_MOSI	- PB15
* 	SPI2_MISO	- PB14
*
